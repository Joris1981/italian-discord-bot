...
class Frasi(commands.Cog):
    ...
    async def start_frasi_game(self, ctx):
        logging.info(f"Commando !frasi opgeroepen door {ctx.author.name}")
    ...
    async def toon_leaderboard(self, ctx):
        logging.info(f"Commando !frasi-leaderboard opgeroepen door {ctx.author.name}")
    ...
    async def speelstatistiek(self, ctx):
        logging.info(f"Commando !frasi-speelstatistiek opgeroepen door {ctx.author.name}")
        week = weeknummer()
        pad = f"{SCORE_PATH}/week_{week}.json"
        if not os.path.exists(pad):
    ...
    async def gemiddelde_tijd(self, ctx):
        logging.info(f"Commando !frasi-gemiddelde opgeroepen door {ctx.author.name}")
        alles = laad_scores()
        if not alles:
            await ctx.send("📊 Nessun dato disponibile.")
            return
        tekst = "⏱️ **Tempo medio di gioco:**\n"
        for gegevens in alles.values():
            if gegevens["beurten"] > 0:
                gemiddeld = int(gegevens["tijd"] / gegevens["beurten"])
                tekst += f"• {gegevens['naam']}: {gemiddeld} sec\n"
        await ctx.send(tekst)
    ...
    async def weekelijkse_herinnering(self):
        logging.info("⏰ Automatische herinnering gestart")
